...
<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜclawxなのか">なぜClawPlusなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**ClawPlus**は、強力なAIエージェントと日常のユーザーとの間のギャップを埋めます。[OpenClaw](https://github.com/OpenClaw)をベースに構築されており、コマンドラインによるAIオーケストレーションを、アクセスしやすく美しいデスクトップ体験に変換します。ターミナルは不要です。

ワークフローの自動化、AI搭載チャネルの管理、インテリジェントなタスクのスケジューリングなど、ClawPlusはAIエージェントを効果的に活用するために必要なインターフェースを提供します。

ClawPlusはベストプラクティスのモデルプロバイダーが事前設定されており、Windowsおよび多言語設定をネイティブにサポートしています。もちろん、**設定 → 詳細設定 → 開発者モード**から高度な設定を微調整することもできます。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。ClawPlusはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**
...
ClawPlusは公式の**OpenClaw**コアを直接ベースに構築されています（現在のバンドルバージョン：**v2026.3.13**）。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
複数のAIエージェントを作成・設定・切り替えできます。各エージェントは独自のモデル、スキル、SOUL.md パーソナリティ定義を持つことができます。リーダー/サブの役割による**企業スタイルの階層構造**をサポートし、インタラクティブな**組織図**で可視化します。**すりガラス風の設定ダイアログ**で名前、ツール権限、**エージェントごとのスキル許可リスト**、チャンネルバインドを管理できます。**組み込みテンプレート**（コーディング、リサーチ、ライティング、DevOps、データ、クリエイティブ）から素早くエージェントを作成するか、完全な設定を**インポート**できます。特定のエージェントをチャンネルや定期タスクに割り当てて精密な制御が可能です。

### 📡 マルチチャネル管理

複数のAIチャネルを同時に設定・監視できます。同じチャネルタイプで**複数のボットアカウント**を運用可能（例：異なるエージェント用に2つのTelegramボット）。Telegramグループ IDは自動検出され、グループごとの**@メンション**権限を設定できます。各チャネルは独立して動作し、専用のエージェントバインドが可能です。

### ⏰ Cronベースの自動化

AIタスクを自動的に実行するようスケジュール設定できます。トリガーを定義し、間隔を設定し、各スケジュールタスクに**特定のエージェントを割り当て**られます。手動介入なしにAIエージェントを24時間稼働させることができます。
...
複数のAIプロバイダー（OpenAI、Anthropic、Google、Moonshot、MiniMaxなど）に接続でき、資格情報はシステムのネイティブキーチェーンに安全に保存されます。OpenAIの**ブラウザベースOAuth**認証と従来のAPIキー認証の両方をサポートしています。

### 🌙 アダプティブテーマ

ライトモード、ダークモード、またはシステム同期テーマ。ClawPlusはあなたの好みに自動的に適応します。
...
[Releases](https://github.com/ValueCell-ai/ClawPlus/releases)ページから、お使いのプラットフォーム向けの最新リリースをダウンロードしてください。
...
ClawPlusを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：

1. **言語と地域** – 使用する言語・地域の設定
2. **環境チェック** – システム要件の確認（Node.js、OpenClawパッケージ、Gatewayサービス）。ウィザードはnpmレジストリから新しいOpenClawバージョンが利用可能かどうかを自動的にチェックします。アップデートが見つかった場合、リスク警告とともにプロンプトが表示され、リアルタイムのプログレスバー付きでアップデートするか、スキップして現在のバージョンを使用し続けるかを選択できます。
3. **AIプロバイダー** – サポートされているプロバイダーのAPIキーを入力
4. **スキルバンドル** – 一般的なユースケース向けの事前設定スキルを選択
5. **検証** – メインインターフェースに入る前に設定をテスト

### プロキシ設定

ClawPlusには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、ClawPlusは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- ClawPlusはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。

---

## アーキテクチャ

ClawPlusは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：
...
保存または実行のタイミングでエディタがノード配置を自動で整え、分岐や実行経路を追いやすくします。
AI 生成ダイアログを開いて任意の設定済みエージェントを選び、対話しながらワークフロー草案を作成し、プレビューしてから新規ワークフローとしてワンクリックで取り込めます。草案が skill や他の外部機能を追加しようとした場合は、取り込み前に承認フローへ切り替わり、あなたの同意後にのみ反映されます。
ワークフロー生成と承認フローの実装メモは [`docs/workflow-ai-generation-analysis.md`](docs/workflow-ai-generation-analysis.md) にまとめています。
...
このリポジトリには `landing/` 配下に独立したランディングページプロジェクトも含まれています。Electron デスクトップ本体とは別の Vite アプリとして、製品紹介ページ、リリースページ、マーケティング用途に個別開発できます。

```bash
pnpm --dir landing dev    # ランディングページをローカル起動
pnpm --dir landing build  # デプロイ用にランディングページをビルド
```
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がClawPlusをより良くするのに役立ちます。
...
ClawPlusは優れたオープンソースプロジェクトの上に構築されています：
...
ClawPlus パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に ClawPlus を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、ClawPlus チームは技術サポート、カスタマイズ、統合を全面的に提供します。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/ClawPlus&type=Date" alt="スター履歴チャート" />
</p>
...
ClawPlusは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
