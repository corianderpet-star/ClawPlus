...
<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#为什么选择-clawx">为什么选择 ClawPlus</a> •
  <a href="#快速上手">快速上手</a> •
  <a href="#系统架构">系统架构</a> •
  <a href="#开发指南">开发指南</a> •
  <a href="#参与贡献">参与贡献</a>
...
**ClawPlus** 是连接强大 AI 智能体与普通用户之间的桥梁。基于 [OpenClaw](https://github.com/OpenClaw) 构建，它将命令行式的 AI 编排转变为易用、美观的桌面体验——无需使用终端。

无论是自动化工作流、连接通讯软件，还是调度智能定时任务，ClawPlus 都能提供高效易用的图形界面，帮助你充分发挥 AI 智能体的能力。

ClawPlus 预置了最佳实践的模型供应商配置，原生支持 Windows 平台以及多语言设置。当然，你也可以通过 **设置 → 高级 → 开发者模式** 来进行精细的高级配置。
...
构建 AI 智能体不应该需要精通命令行。ClawPlus 的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**
...
ClawPlus 直接基于官方 **OpenClaw** 核心构建（当前内置版本：**v2026.3.13**）。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
创建、配置和切换多个 AI 智能体——每个智能体可拥有独立的模型、技能和 SOUL.md 人格定义。支持**公司架构式层级**（主管/执行角色），并以交互式**组织架构图**可视化展示。通过**毛玻璃风格设置对话框**管理名称、工具权限、**独立技能白名单**和频道绑定。支持从**内置模板**（编程、研究、写作、DevOps、数据、创意）快速创建智能体，或通过**导入功能**加载完整配置。可将特定智能体分配到频道和定时任务中进行精细控制。

### 📡 多频道管理

同时配置和监控多个 AI 频道。支持同一频道类型运行**多个机器人账号**（如为不同智能体各配一个 Telegram 机器人）。Telegram 群组 ID 自动识别并支持按群配置 **@提及** 权限。每个频道独立运行，可绑定专属智能体。

### ⏰ 定时任务自动化

调度 AI 任务自动执行。定义触发器、设置时间间隔，并**指定特定智能体**执行每个定时任务。让 AI 智能体 7×24 小时不间断工作。
...
连接多个 AI 供应商（OpenAI、Anthropic、Google、Moonshot、MiniMax 等），凭证安全存储在系统原生密钥链中。支持 OpenAI **浏览器 OAuth 登录**，兼容传统 API Key 认证方式。

### 🌙 自适应主题

支持浅色模式、深色模式或跟随系统主题。ClawPlus 自动适应你的偏好设置。
...
从 [Releases](https://github.com/ValueCell-ai/ClawPlus/releases) 页面下载适用于你平台的最新版本。
...
首次启动 ClawPlus 时，**设置向导** 将引导你完成以下步骤：

1. **语言与区域** – 配置你的首选语言和地区
2. **环境检查** – 验证系统要求（Node.js、OpenClaw 包、Gateway 服务）。向导会自动从 npm 仓库检测是否有更新的 OpenClaw 版本可用。若检测到新版本，将提示更新风险警告，你可以选择立即更新（带实时进度条）或跳过并继续使用当前版本。
3. **AI 供应商** – 输入所支持供应商的 API 密钥
4. **技能包** – 选择适用于常见场景的预配置技能
5. **验证** – 在进入主界面前测试你的配置

> Moonshot（Kimi）说明：ClawPlus 默认保持开启 Kimi 的 web search。  
> 当配置 Moonshot 后，ClawPlus 也会将 OpenClaw 配置中的 Kimi web search 同步到中国区端点（`https://api.moonshot.cn/v1`）。

### 代理设置

ClawPlus 内置了代理设置，适用于需要通过本地代理客户端访问外网的场景，包括 Electron 本身、OpenClaw Gateway，以及 Telegram 这类频道的联网请求。
...
- 如果启用了 Telegram，ClawPlus 还会把代理同步到 OpenClaw 的 Telegram 频道配置中。

---

## 系统架构

ClawPlus 采用 **双进程 + Host API 统一接入架构**。渲染进程只调用统一客户端抽象，协议选择与进程生命周期由 Electron 主进程统一管理：
...
在保存或运行工作流时，编辑器会自动整理节点布局，让分支关系和执行路径更直观。
现在也支持打开 AI 生成对话框，选择任意已配置智能体，通过对话生成工作流草稿、预览流程图，并一键导入为新的工作流。当草稿准备补充 skill 或其他外部能力时，系统会先进入审批流，只有在你明确同意后才会继续导入。
工作流生成与审批流的实现解析见 [`docs/workflow-ai-generation-analysis.md`](docs/workflow-ai-generation-analysis.md)。
...
仓库中还包含一个位于 `landing/` 的独立落地页项目。它是单独的 Vite 应用，适合做官网首页、发布页或营销页面，不依赖 Electron 主程序一起开发。

```bash
pnpm --dir landing dev    # 本地启动落地页
pnpm --dir landing build  # 构建可部署的落地页产物
```
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 ClawPlus 变得更好。
...
我们正在启动 ClawPlus 合作伙伴计划，寻找能够帮助我们将 ClawPlus 介绍给更多客户的合作伙伴，尤其是那些有定制化 AI 智能体或自动化需求的客户。

合作伙伴负责帮助我们连接潜在用户和项目，ClawPlus 团队则提供完整的技术支持、定制开发与集成服务。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/ClawPlus&type=Date" alt="Stars 历史图表" />
</p>
...
ClawPlus 基于 [MIT 许可证](LICENSE) 发布。你可以自由地使用、修改和分发本软件。
